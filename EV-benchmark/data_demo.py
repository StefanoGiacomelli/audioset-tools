...
def compute_stats_from_dataloader(dl: DataLoader, sample_rate: int):
    ...
    for batch in dl:
        ...
        total_batches += 1
        # Compute duration per sample (in sec.) from the number of samples and the sample rate.
...
def process_dataloader(dl, sample_rate: int, dataset_name: str):
    ...
    """
    # If dl is a list of dataloaders, process them all
...
if __name__ == "__main__":
    ...
    audioset_ev_dm = AudioSetEV_DataModule(TP_file="./EV-benchmark/AudioSet-EV/EV_Positives.csv",
                                           TP_folder="./EV-benchmark/AudioSet-EV/Positives_files/",
                                           TN_file="./EV-benchmark/AudioSet-EV/EV_Negatives.csv",
                                           TN_folder="./EV-benchmark/AudioSet-EV/Negatives_files/",
                                           batch_size=32,
    ...
    esc50_dm = ESC50_DataModule(file_path="./EV-benchmark/ESC-50/esc50.csv",
                                folder_path="./EV-benchmark/ESC-50/cross_val_folds/",
                                target_size=160000,
                                target_sr=32000,
                                batch_size=32)
    esc50_dm.setup()
    ...
    sirennet_dm = sireNNet_DataModule(folder_path="./EV-benchmark/sireNNet/",
                                      target_sr=32000,
                                      target_size=96000,
                                      batch_size=32)
    sirennet_dm.setup()
    ...
    urbansound_dm = UrbanSound8K_DataModule(folder_path="./EV-benchmark/UrbanSound8K/audio",
                                            metadata_path="./EV-benchmark/UrbanSound8K/metadata/UrbanSound8K.csv",
                                            target_sr=32000,
                                            min_length=32000,
                                            batch_size=32)
    urbansound_dm.setup()
    ...
    fsd50k_dm = FSD50K_DataModule(pos_file="./EV-benchmark/FSD50K/FSD-eval_positives.csv",
                                  neg_file="./EV-benchmark/FSD50K/FSD-eval_negatives.csv",
                                  folder_path="./EV-benchmark/FSD50K/FSD50K.eval_audio/",
                                  target_sr=32000,
                                  batch_size=32)
    fsd50k_dm.setup()
